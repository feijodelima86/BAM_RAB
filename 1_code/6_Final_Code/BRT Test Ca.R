...
Ca_DF <- (COMPARTMENTS_AVG[,c("SAMPLE_DESCRIPTOR","spm_Ca","colloidal_Ca","trulydissolved_Ca","TURNOVER","Car","OM.AREA.g.m2")])

Ca_TRD <- (Ca_DF[,c(1,4,5,7,6)])

names(Ca_TRD)<- c("SAMPLE_DESCRIPTOR","Ca_TRD", "TURNOVER", "OM", "Ca")

Ca_TRD<-Ca_TRD[Ca_TRD$Ca_TRD>0,]

Ca_TRD<-Ca_TRD[Ca_TRD$OM>0,]

#Ca_TRD$Ca_TRD <- log10(Ca_TRD$Ca_TRD+1)

#Ca_TRD$Ca <- log10(Ca_TRD$Ca+1)

#Ca_TRD$OM <- log10(Ca_TRD$OM+1)
...
UCFR.SS.tc5.lr002 <- gbm.step(data=Ca_TRD, 
                              gbm.x = c(2,3,4),
                              gbm.y = 5,
                              family = "gaussian",
                              tree.complexity = 2,
...
CHL_DF<-data.frame(UCFR.SS.tc5.lr002$fitted, Ca_TRD$Ca)

predfit<-lm(UCFR.SS.tc5.lr002$fitted ~ Ca_TRD$Ca)

summary(predfit)

par(mfrow=c(1,1))

plot(Ca_TRD$Ca, UCFR.SS.tc5.lr002$fitted, xlab="Obs", ylab="Fitted Values", 
     abline(a=0, b=1),
     lwd=1.5,        
     las=1,
     font.lab=2,
