...
newx <- seq(min(COMPARTMENTS$TD.As[!is.na(COMPARTMENTS$TD.As)]), max(COMPARTMENTS$TD.As[!is.na(COMPARTMENTS$TD.As)]), length.out=100)
preds <- predict(EPIL.TD.As, newdata = data.frame(TD.As=newx), interval = 'confidence')

COMPARTMENTS$TD.As[!is.na(COMPARTMENTS$TD.As)]
