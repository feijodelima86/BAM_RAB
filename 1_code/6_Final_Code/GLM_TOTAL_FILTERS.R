...
Al_DF <- (COMPARTMENTS_AVG[,c("W_Al","F_Al","UF_Al","TURNOVER","Al")])
...
mix.int_Al_SPM <- glm(Al ~ (Al_SPM), data = Al_SPM, family=gaussian)

mix.int_Al_COL <- glm(Al ~ (Al_COL * TURNOVER), data = Al_COL, family=gaussian)

mix.int_Al_TRD <- glm(Al ~ (TURNOVER), data = Al_TRD, family=gaussian)
...
As_DF <- (COMPARTMENTS_AVG[,c("W_As_Oshift","F_As_Oshift","UF_As_Oshift","TURNOVER","As")])
...
Ca_DF <- (COMPARTMENTS_AVG[,c("W_Ca","F_Ca","UF_Ca","TURNOVER","Car")])
...
mix.int_Ca_SPM <- glm(Ca ~ (Ca_SPM), data = Ca_SPM, family=gaussian)

mix.int_Ca_COL <- glm(Ca ~ (Ca_COL + TURNOVER), data = Ca_COL, family=gaussian)

mix.int_Ca_TRD <- glm(Ca ~ (Ca_TRD), data = Ca_TRD, family=gaussian)
...
Cd_DF <- (COMPARTMENTS_AVG[,c("W_Cd114","F_Cd114","trulydissolved_Cd114","TURNOVER","Cd")])
...
mix.int_Cd_COL <- glm(Cd ~ (Cd_COL), data = Cd_COL, family=gaussian)
...
Co_DF <- (COMPARTMENTS_AVG[,c("W_Co","F_Co","trulydissolved_Co","TURNOVER","Co")])
...
plot(density(Co_SPM$Co_SPM, bw=0.0029), ylim=c(0,120), xlim=c(0,0.04), col="Purple", lwd=2)
lines(density(Co_COL$Co_COL, bw=0.0029), col="Blue", lwd=2)
lines(density(Co_TRD$Co_TRD, bw=0.0029), col="LightBlue", lwd=2)
box(lwd=2)

mix.int_Co_SPM <- glm(Co ~ (TURNOVER), data = Co_SPM, family=gaussian)

mix.int_Co_COL <- glm(Co ~ (TURNOVER), data = Co_COL, family=gaussian)

mix.int_Co_TRD <- glm(Co ~ (TURNOVER), data = Co_TRD, family=gaussian)
...
Cr_DF <- (COMPARTMENTS_AVG[,c("W_Cr_NH3","F_Cr_NH3","trulydissolved_Cr_NH3","TURNOVER","Cr")])
...
mix.int_Cr_TRD <- glm(Cr ~ (TURNOVER), data = Cr_TRD, family=gaussian)
...
Cu_DF <- (COMPARTMENTS_AVG[,c("W_Cu_NH3","F_Cu_NH3","trulydissolved_Cu_NH3","TURNOVER","Cu")])
...
mix.int_Cu_SPM <- glm(Cu ~ (TURNOVER), data = Cu_SPM, family=gaussian)
...
Fe_DF <- (COMPARTMENTS_AVG[,c("W_Fe_NH3","F_Fe_NH3","trulydissolved_Fe_NH3","TURNOVER","Fer.1")])
...
Mo_DF <- (COMPARTMENTS_AVG[,c("W_Mo_Oshift","F_Mo_Oshift","trulydissolved_Mo_Oshift","TURNOVER","Mo")])
...
Mn_DF <- (COMPARTMENTS_AVG[,c("W_Mn","F_Mn","trulydissolved_Mn","TURNOVER","Mn")])
...
mix.int_Mn_SPM <- glm(Mn ~ (Mn_SPM), data = Mn_SPM, family=gaussian)

mix.int_Mn_COL <- glm(Mn ~ (Mn_COL), data = Mn_COL, family=gaussian)

mix.int_Mn_TRD <- glm(Mn ~ (Mn_TRD), data = Mn_TRD, family=gaussian)
...
Ni_DF <- (COMPARTMENTS_AVG[,c("W_Ni","F_Ni","trulydissolved_Ni","TURNOVER","Ni")])
...
plot(density(Ni_SPM$Ni_SPM, bw=0.01198), ylim=c(0,15), xlim=c(0,0.4), col="Purple", lwd=2)
lines(density(Ni_COL$Ni_COL, bw=0.01198), col="Blue", lwd=2)
lines(density(Ni_TRD$Ni_TRD, bw=0.01198), col="LightBlue", lwd=2)
box(lwd=2)

mix.int_Ni_SPM <- glm(Ni ~ (Ni_SPM), data = Ni_SPM, family=gaussian)

mix.int_Ni_COL <- glm(Ni ~ (Ni_COL), data = Ni_COL, family=gaussian)

mix.int_Ni_TRD <- glm(Ni ~ (Ni_TRD), data = Ni_TRD, family=gaussian)
...
P_DF <- (COMPARTMENTS_AVG[,c("W_P","F_P","trulydissolved_P","TURNOVER","P")])
...
mix.int_P_SPM <- glm(P ~ (TURNOVER), data = P_SPM, family=gaussian)
...
Pb_DF <- (COMPARTMENTS_AVG[,c("W_Pb_NH3","F_Pb_NH3","trulydissolved_Pb_NH3","TURNOVER","Pb")])
...
mix.int_Pb_SPM <- glm(Pb ~ (TURNOVER), data = Pb_SPM, family=gaussian)
...
S_DF <- (COMPARTMENTS_AVG[,c("W_S","F_S","trulydissolved_S","TURNOVER","S")])
...
Se_DF <- (COMPARTMENTS_AVG[,c("W_Se82_Oshift","F_Se82_Oshift","trulydissolved_Se82_Oshift","TURNOVER","Se")])
...
plot(density(Se_SPM$Se_SPM, bw=0.01358), ylim=c(0,25), xlim=c(0,0.20), col="Purple", lwd=2)
lines(density(Se_COL$Se_COL, bw=0.01358), col="Blue", lwd=2)
lines(density(Se_TRD$Se_TRD, bw=0.01358), col="LightBlue", lwd=2)
box(lwd=2)

mix.int_Se_SPM <- glm(Se ~ (TURNOVER), data = Se_SPM, family=gaussian)

mix.int_Se_COL <- glm(Se ~ (TURNOVER), data = Se_COL, family=gaussian)

mix.int_Se_TRD <- glm(Se ~ (TURNOVER), data = Se_TRD, family=gaussian)
...
Zn_DF <- (COMPARTMENTS_AVG[,c("W_Zn","F_Zn","trulydissolved_Zn","TURNOVER","Zn")])
...
mix.int_Zn_COL <- glm(Zn ~ (Zn_COL), data = Zn_COL, family=gaussian)

mix.int_Zn_TRD <- glm(Zn ~ (TURNOVER), data = Zn_TRD, family=gaussian)
