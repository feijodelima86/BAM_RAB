...
Al_DF <- (COMPARTMENTS_AVG[,c("spm_Al","colloidal_Al","trulydissolved_Al","TURNOVER","Al","OM.AREA.g.m2")])
...
mix.int_Ca_SPM <- glm(Ca ~ (TURNOVER), data = Ca_SPM, family=gaussian)

mix.int_Ca_COL <- glm(Ca ~ (TURNOVER), data = Ca_COL, family=gaussian)
...
Cd_DF <- (COMPARTMENTS_AVG[,c("spm_Cd114","colloidal_Cd114","trulydissolved_Cd114","TURNOVER","Cd","OM.AREA.g.m2")])
...
Cd_DF$cat <- cut(Cd_DF$TURNOVER, 
                 breaks=c(-Inf, 0.3587456     , 0.4699721     , 0.5016606   , 0.5487757, 0.5981569   , Inf), 
                 labels=c("NA", "0.21 - 0.46","0.46 - 0.50","0.50 - 0.54","0.54 - 0.59","0.59 - 0.76"))

### Cd_TRD ###

Cd_TRD <- (Cd_DF[,c(3,4,5,6,7)])

names(Cd_TRD)<- c("Cd_TRD", "TURNOVER", "Cd", "OM", "cat")
...
mix.int_Cd_TRD <- glm(Cd ~ (Cd_TRD + OM), data = Cd_TRD, family=gaussian)
...
mix.int_Mn_SPM <- glm(Mn ~ (TURNOVER), data = Mn_SPM, family=gaussian)
