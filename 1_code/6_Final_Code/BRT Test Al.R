...
Al_DF <- (COMPARTMENTS_AVG[,c("SAMPLE_DESCRIPTOR","spm_Al","colloidal_Al","colloidal_Al","TURNOVER","Al","OM.AREA.g.m2","OM.AREA.g.m2")])

Al_TRD <- (Al_DF[,c(1,4,5,7,6)])

names(Al_TRD)<- c("SAMPLE_DESCRIPTOR","Al_TRD", "TURNOVER", "OM", "Al")

Al_TRD<-Al_TRD[Al_TRD$Al_TRD>0,]

Al_TRD<-Al_TRD[Al_TRD$OM>0,]

#Al_TRD$Al_TRD <- log10(Al_TRD$Al_TRD+1)

#Al_TRD$Al <- log10(Al_TRD$Al+1)

#Al_TRD$OM <- log10(Al_TRD$OM+1)
...
UCFR.SS.tc5.lr002 <- gbm.step(data=Al_TRD, 
                              gbm.x = c(2,3,4),
                              gbm.y = 5,
                              family = "gaussian",
                              tree.complexity = 2,
...
CHL_DF<-data.frame(UCFR.SS.tc5.lr002$fitted, Al_TRD$Al)

predfit<-lm(UCFR.SS.tc5.lr002$fitted ~ Al_TRD$Al)

summary(predfit)

par(mfrow=c(1,1))

plot(Al_TRD$Al, UCFR.SS.tc5.lr002$fitted, xlab="Obs", ylab="Fitted Values", 
     abline(a=0, b=1),
     lwd=1.5,        
     las=1,
     font.lab=2,
