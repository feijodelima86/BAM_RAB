...
Al_DF <- Al_DF[complete.cases(Al_DF$TURNOVER), ]
...
Al_DF$cat <- cut(Al_DF$TURNOVER, 
                 breaks=c(-Inf, 0.1679231    , 0.2365162     , 0.2587824    , 0.2922810 , 0.3430609    , Inf), 
                 labels=c("NA", "0.16 - 0.23","0.23 - 0.25","0.25 - 0.29","0.29 - 0.34","0.34 - 0.43"))
...
As_DF <- As_DF[complete.cases(As_DF$TURNOVER), ]
...
Ca_DF <- Ca_DF[complete.cases(Ca_DF$TURNOVER), ]
...
Cd_DF <- Cd_DF[complete.cases(Cd_DF$TURNOVER), ]
...
Co_DF <- Co_DF[complete.cases(Co_DF$TURNOVER), ]
...
Cr_DF <- Cr_DF[complete.cases(Cr_DF$TURNOVER), ]
...
Cu_DF <- Cu_DF[complete.cases(Cu_DF$TURNOVER), ]
...
mix.int_Cu_COL <- glm(Cu ~ (Cu_COL), data = Cu_COL, family=gaussian)
...
Fe_DF <- Fe_DF[complete.cases(Fe_DF$TURNOVER), ]
...
mix.int_Fe_TRD <- glm(Fe ~ (Fe_TRD), data = Fe_TRD, family=gaussian)
...
Mo_DF <- Mo_DF[complete.cases(Mo_DF$TURNOVER), ]
...
mix.int_Mo_TRD <- glm(Mo ~ (Mo_TRD), data = Mo_TRD, family=gaussian)
...
Mn_DF <- Mn_DF[complete.cases(Mn_DF$TURNOVER), ]
...
Ni_DF <- Ni_DF[complete.cases(Ni_DF$TURNOVER), ]
...
P_DF <- P_DF[complete.cases(P_DF$TURNOVER), ]
...
Pb_DF <- Pb_DF[complete.cases(Pb_DF$TURNOVER), ]
...
mix.int_Pb_TRD <- glm(Pb ~ (Pb_TRD), data = Pb_TRD, family=gaussian)
...
S_DF <- S_DF[complete.cases(S_DF$TURNOVER), ]
...
mix.int_S_TRD <- glm(S ~ (TURNOVER), data = S_TRD, family=gaussian)
