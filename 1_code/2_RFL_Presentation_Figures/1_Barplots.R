...
ALL.SUM<-data.frame(aggregate(as.numeric(dfALL[,n1]) ~ SAMPLING_DATE+SITE+SAMPLE_DESCRIPTOR, dfALL, mean))
ALL.SUM[,5]<-aggregate(as.numeric(dfALL[,n1]) ~ SAMPLING_DATE+SITE+SAMPLE_DESCRIPTOR, dfALL, se)[,4]
ALL.SUM <- subset(ALL.SUM, SITE == ssite)
names(ALL.SUM)<-c("DATE","SITE","SAMPLE_DESCRIPTOR","MEAN.ALL","STDER.ALL")
ALL.SUM[,6]<--2
ALL.SUM[,c(4,5)]<-ALL.SUM[,c(4,5)]*mult 
