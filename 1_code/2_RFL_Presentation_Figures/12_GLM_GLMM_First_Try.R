...
mix.int <- glm(As ~ SAMPLING_DATE * SITE_DISTANCE, data = BIG5.GLM, 
               family=gaussian)

mix.int.a <- glm(As ~ SAMPLING_DATE * SITE, data = BIG5.GLM, 
               family=gaussian)

mod.names <- c('mix.int', 'mix.int.a')

models <- list(mix.int, mix.int.a)
...
plot2<- ggplot(BIG5.GLM, aes(SAMPLING_DATE, As, col = SITE_DISTANCE))+
  geom_point() +
  geom_line(data=pframe)+
  scale_y_continuous(limits = c(NA,max(COMPARTMENTS[COMPARTMENTS$ROWNUM != 1, ]$As.1, na.rm = T)), 
