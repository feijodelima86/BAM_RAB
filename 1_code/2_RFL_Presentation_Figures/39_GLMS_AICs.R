...
with(summary(As_TD), 1 - deviance/null.deviance)

As_TD <- glm(As ~ UF_As_Oshift, data = COMPARTMENTS_AVG, family=gaussian)

no.int_As_TD  <- glm(As ~ (UF_As_Oshift + TURNOVER), data = COMPARTMENTS_AVG, family=gaussian)

mix.int_As_TD <- glm(As ~ (UF_As_Oshift * TURNOVER), data = COMPARTMENTS_AVG, family=gaussian)

mix.int_As_TD2 <- glm(As ~ (UF_As_Oshift * TURNOVER), data = COMPARTMENTS_AVG, family=gaussian)
...
aictab(c(models), modnames=c("As_TD","mix.int_As_TD","mix.int_As_TD2","no.int_As_TD"))

with(summary(As_TD), 1 - deviance/null.deviance)

plot(allEffects(mix.int_As_TD))
, lines = list(multiline = T), confint = list(style = "auto"))

Anova(mix.int_As_TD)

### Cd ###

Cd_TD <- glm(Cd ~ UF_Cd114 * SAMPLE_DESCRIPTOR, data = COMPARTMENTS_AVG, family=gaussian)

summary(Cd_TD)

with(summary(Cd_TD), 1 - deviance/null.deviance)

no.int_Cd_TD    <- glm(Cd ~ (UF_Cd114 + TURNOVER) : SAMPLE_DESCRIPTOR, data = COMPARTMENTS_AVG, family=gaussian)

mix.int_Cd_TD   <- glm(Cd ~ (UF_Cd114 * TURNOVER), data = COMPARTMENTS_AVG, family=gaussian)

mix.int_Cd_TD2  <- glm(Cd ~ (UF_Cd114 * TURNOVER) * SAMPLE_DESCRIPTOR, data = COMPARTMENTS_AVG, family=gaussian)
...
aictab(c(models), modnames=c("Cd_TD","mix.int_Cd_TD","mix.int_Cd_TD2","no.int_Cd_TD"))
...
Cu_TD <- glm(Cu ~ UF_Cu_NH3 * SAMPLE_DESCRIPTOR, data = COMPARTMENTS_AVG, family=gaussian)

summary(Cu_TD)

with(summary(Cu_TD), 1 - deviance/null.deviance)

no.int_Cu_TD    <- glm(Cu ~ (UF_Cu_NH3 + TURNOVER) : SAMPLE_DESCRIPTOR, data = COMPARTMENTS_AVG, family=gaussian)

mix.int_Cu_TD   <- glm(Cu ~ (UF_Cu_NH3 * TURNOVER), data = COMPARTMENTS_AVG, family=gaussian)

mix.int_Cu_TD2  <- glm(Cu ~ (UF_Cu_NH3 * TURNOVER) * SAMPLE_DESCRIPTOR, data = COMPARTMENTS_AVG, family=gaussian)
...
with(summary(no.int_Cu_TD), 1 - deviance/null.deviance)
...
Fe_TD <- glm(Fer.1 ~ UF_Fe_NH3 : SAMPLE_DESCRIPTOR, data = COMPARTMENTS_AVG, family=gaussian)

summary(Cu_TD)

with(summary(Cu_TD), 1 - deviance/null.deviance)

no.int_Fe_TD    <- glm(Fer.1 ~ (UF_Fe_NH3 + TURNOVER) : SAMPLE_DESCRIPTOR, data = COMPARTMENTS_AVG, family=gaussian)

mix.int_Fe_TD   <- glm(Fer.1 ~ (UF_Fe_NH3 * TURNOVER), data = COMPARTMENTS_AVG, family=gaussian)

mix.int_Fe_TD2  <- glm(Fer.1 ~ (UF_Fe_NH3 * TURNOVER) * SAMPLE_DESCRIPTOR, data = COMPARTMENTS_AVG, family=gaussian)
...
aictab(c(models), modnames=c("Fe_TD","mix.int_Fe_TD","mix.int_Fe_TD2","no.int_Fe_TD"))
...
Mo_TD <- glm(Mo ~ UF_Mo_Oshift : SAMPLE_DESCRIPTOR, data = COMPARTMENTS_AVG, family=gaussian)

summary(Mo_TD)

with(summary(Mo_TD), 1 - deviance/null.deviance)

no.int_Mo_TD    <- glm(Mo ~ (UF_Mo_Oshift + TURNOVER) : SAMPLE_DESCRIPTOR, data = COMPARTMENTS_AVG, family=gaussian)

mix.int_Mo_TD   <- glm(Mo ~ (UF_Mo_Oshift * TURNOVER), data = COMPARTMENTS_AVG, family=gaussian)

mix.int_Mo_TD2  <- glm(Mo ~ (UF_Mo_Oshift * TURNOVER) * SAMPLE_DESCRIPTOR, data = COMPARTMENTS_AVG, family=gaussian)
...
aictab(c(models), modnames=c("Mo_TD","mix.int_Mo_TD","mix.int_Mo_TD2","no.int_Mo_TD"))

with(summary(no.int_Mo_TD), 1 - deviance/null.deviance)
...
P_TD <- glm(P ~ UF_P : SAMPLE_DESCRIPTOR, data = COMPARTMENTS_AVG, family=gaussian)

summary(P_TD)

with(summary(P_TD), 1 - deviance/null.deviance)

no.int_P_TD    <- glm(P ~ (UF_P + TURNOVER) : SAMPLE_DESCRIPTOR, data = COMPARTMENTS_AVG, family=gaussian)

mix.int_P_TD   <- glm(P ~ (UF_P * TURNOVER), data = COMPARTMENTS_AVG, family=gaussian)

mix.int_P_TD2  <- glm(P ~ (UF_P * TURNOVER) * SAMPLE_DESCRIPTOR, data = COMPARTMENTS_AVG, family=gaussian)
...
aictab(c(models), modnames=c("P_TD","mix.int_P_TD","mix.int_P_TD2","no.int_P_TD"))

with(summary(no.int_P_TD), 1 - deviance/null.deviance)
